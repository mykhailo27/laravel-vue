...
const table_actions = [
    {
        ...
        route_name: 'api.packages.delete-multiple',
        method: 'delete'
    }
];
...
const row_actions = [
    {
        ...
        route_name: 'api.packages.delete',
        method: 'delete',
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <template #header>
            <Header :link="{url: route('packages.create'), name: 'Create'}"
                    :filters="filters" :can_create="can[ability.CREATE]"/>
        </template>

        <Table table_id="package-table" :columns="['id', 'name', 'desc', 'state', 'created_at', 'actions']" :data="packages"
               :details_url="route('packages.details', {package: '__ROW_ID__'})"
